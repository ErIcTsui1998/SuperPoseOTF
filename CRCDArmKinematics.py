import numpy as np
from utils import get_rigid_transform, GetPositionInBaseFrame, QuaternionToRot
from Kinematics import dvrk_DH_transformation
EPS=0.00001

class ArmKinematicsData:
    def __init__(self):
        self.JointHistory = np.zeros(3) # joint history
        self.PosLocalCartesianHistory = np.zeros(3) # j6 position in robot base frame
        self.PosInEyeCartesianHistory = np.zeros(3) # j6 position in camera frame
        self.RotLocalCartesianHistory = np.zeros(4) # orientation in robot base frame (x,y,z,w)
        self.RotInEyeCartesianHistory = np.zeros(4) # orientation in camera frame (x,y,z,w)
        self.TimeStamp = np.zeros(1)
        self.T_rcm_aruco = np.identity(4) # eye-to-hand transformation matrix
        self.T_c_rcm_list = []
        self.AXYB_obj = Batch_Processing()

    def ReadJointHistory(self, JH):
        self.JointHistory = JH

    def ReadPosLocalHistory(self,PLH):
        self.PosLocalCartesianHistory = PLH
    
    def ReadPosInEyeHistory(self,PEH):
        self.PosInEyeCartesianHistory = PEH
    
    def ReadRotLocalHistory(self, RLH):
        self.RotLocalCartesianHistory = RLH
    
    def ReadRotInEyeHistory(self, REH):
        self.RotInEyeCartesianHistory = REH
    
    def ReadTimeStamp(self, TimeStamp):
        self.TimeStamp = TimeStamp
    
    def UpdateT_rcm_aruco(self):
        Pos_rcm_list = [GetPositionInBaseFrame(self.JointHistory[i], np.zeros(3), 6) for i in range(len(self.PosLocalCartesianHistory))]
        Pos_Aruco_list = self.PosLocalCartesianHistory
        T_rcm_aruco = get_rigid_transform(Pos_Aruco_list, Pos_rcm_list)
        self.T_rcm_aruco = T_rcm_aruco
    
    def UpdateT_c_rcm_list(self):
        n_data = len(self.JointHistory)
        self.T_c_rcm_list = []
        for i in range(n_data):
            js = self.JointHistory[i]
            T_rcm_j6 = dvrk_DH_transformation(js, end_joint=6)
            T_c_j6_bespoke = np.identity(4)
            T_aruco_j6_bespoke = np.identity(4)
            pos_j6_cam = self.PosInEyeCartesianHistory[i]
            pos_j6_Aruco_base = self.PosLocalCartesianHistory[i]
            R_aruco_j6 = QuaternionToRot(self.RotLocalCartesianHistory[i])
            R_c_j6 = QuaternionToRot(self.RotInEyeCartesianHistory[i])
            
            T_c_j6_bespoke[:3,:3] = R_c_j6
            T_c_j6_bespoke[:3,-1] = pos_j6_cam
            T_aruco_j6_bespoke[:3,:3] = R_aruco_j6
            T_aruco_j6_bespoke[:3,-1] = pos_j6_Aruco_base

            # T_c_rcm = T_c_j6_bespoke @ np.linalg.inv(T_aruco_j6_bespoke) @ np.linalg.inv(self.T_rcm_aruco)
            T_c_rcm = T_c_j6_bespoke @ np.linalg.inv(T_rcm_j6)
            self.T_c_rcm_list.append(T_c_rcm)
        
        # AX=YB solver method
        A_mat_list = np.zeros((4,4,n_data))
        B_mat_list = np.zeros((4,4,n_data))
        for i in range(n_data):
            js = self.JointHistory[i]
            T_rcm_j6 = dvrk_DH_transformation(js, end_joint=6)
            B = np.linalg.inv(T_rcm_j6)
            T_c_j6_bespoke = np.identity(4)
            R_c_j6 = QuaternionToRot(self.RotInEyeCartesianHistory[i])
            pos_j6_cam = self.PosInEyeCartesianHistory[i]
            T_c_j6_bespoke[:3,:3] = R_c_j6
            T_c_j6_bespoke[:3,-1] = pos_j6_cam
            A = np.linalg.inv(T_c_j6_bespoke)
            B_mat_list[:,:,i] = B
            A_mat_list[:,:,i] = A

        X_est,Y_est, Y_est_check,ErrorStats = self.AXYB_obj.pose_estimation(A_mat_list, B_mat_list) # X_est corresponds to T_cam_rcm

        for i in range(n_data):
            self.T_c_rcm_list.append(X_est)


    def UpdateT_c_rcm_list_MTM(self):
        n_data = len(self.PosLocalCartesianHistory)
        self.T_c_rcm_list = []
        for i in range(n_data):
            pos_local = self.PosLocalCartesianHistory[i]
            pos_eye = self.PosInEyeCartesianHistory[i]
            R_06 = QuaternionToRot(self.RotLocalCartesianHistory[i])
            R_c6 = QuaternionToRot(self.RotInEyeCartesianHistory[i])
            R_c0 = R_c6 @ R_06.T
            T_cr = np.identity(4)
            T_cr[:3,:3] = R_c0
            T_cr[:3,-1] = pos_eye - R_c0@pos_local
            self.T_c_rcm_list.append(T_cr)

"""
    + Simultaneous Robot/World and Tool/Flange Calibration:    
    Implementation of Shah, Mili. "Solving the robot-world/hand-eye calibration problem using the Kronecker product." 
    Journal of Mechanisms and Robotics 5.3 (2013): 031007.
    
    Batch_Processing solvesfor  X and Y in AX=YB from a set of (A,B) paired measurements.
    (Ai,Bi) are absolute pose measurements with known correspondance       

    A: (4x4xn) 
    X: (4x4): unknown
    Y: (4x4): unknown
    B: (4x4xn) 
    n number of measurements
    
    + EKF,IEKF solves for AX=XB from a set of (Ai,Bi) relative pose measurements with known correspondance.
    so3 representation was used to represent the state of rotation.  
    
    @author: elif.ayvali
"""

import pickle
import numpy as np
EPS=0.00001

class Batch_Processing:        
    def pose_estimation(self,A,B):
   
        n=A.shape[2];
        T = np.zeros([9,9]);
        X_est= np.eye(4)
        Y_est= np.eye(4)

        #Permutate A and B to get gross motions
        idx = np.random.permutation(n)
        A=A[:,:,idx];
        B=B[:,:,idx];
    
        for ii in range(n-1):   
            Ra = A[0:3,0:3,ii]
            Rb = B[0:3,0:3,ii]
          #  K[9*ii:9*(ii+1),:] = np.concatenate((np.kron(Rb,Ra), -np.eye(9)),axis=1)
            T = T + np.kron(Rb,Ra);
        
        U, S, Vt=np.linalg.svd(T)
        xp=Vt.T[:,0]
        yp=U[:,0]
        X=np.reshape(xp, (3,3), order="F")#F: fortran/matlab reshape order
        Xn = (np.sign(np.linalg.det(X))/ np.abs(np.linalg.det(X))**(1/3))*X
        #re-orthogonalize to guarantee that they are indeed rotations.
        U_n, S_n, Vt_n=np.linalg.svd(Xn)
        X=np.matmul(U_n,Vt_n)
    
        Y=np.reshape(yp, (3,3), order="F")#F: fortran/matlab reshape order
        Yn = (np.sign(np.linalg.det(Y))/ np.abs(np.linalg.det(Y))**(1/3))*Y
        U_yn, S_yn, Vt_yn=np.linalg.svd(Yn)
        Y=np.matmul(U_yn,Vt_yn)
      
        A_est = np.zeros([3*n,6])
        b_est = np.zeros([3*n,1])
        for ii in range(n-1):       
            A_est[3*ii:3*ii+3,:] =np.concatenate((-A[0:3,0:3,ii], np.eye(3)),axis=1)         
            b_est[3*ii:3*ii+3,:] = np.transpose(A[0:3,3,ii] - np.matmul(np.kron(B[0:3,3,ii].T,np.eye(3)), np.reshape(Y, (9,1), order="F")).T)
    
        t_est_np=np.linalg.lstsq(A_est,b_est,rcond=None)
        if t_est_np[2]<A_est.shape[1]: # A_est.shape[1]=6
            print('Rank deficient')
        t_est=t_est_np[0]
        X_est[0:3,0:3]= X
        X_est[0:3,3]= t_est[0:3].T  
        Y_est[0:3,0:3]= Y    
        Y_est[0:3,3]= t_est[3:6].T        
        #verify Y_est using rigid_registration
        Y_est_check,ErrorStats= self.__rigid_registration(A,X_est,B)
        return X_est,Y_est, Y_est_check,ErrorStats
    
    def __rigid_registration(self, A,X,B):
        #nxnx4            
        """solves for Y in YB=AX
        A: (4x4xn) 
        B: (4x4xn) 
        X= (4x4)   
        Y= (4x4)       
        n number of measurements
        ErrorStats: Registration error (mean,std)
        """
        n=A.shape[2];
        AX=np.zeros(A.shape)
        AXp=np.zeros(A.shape)
        Bp=np.zeros(B.shape)
        pAX=np.zeros(B[0:3,3,:].shape)#To calculate reg error    
        pYB=np.zeros(B[0:3,3,:].shape)#To calculate reg error  
        Y_est=np.eye(4)

        ErrorStats=np.zeros((2,1))
        
        for ii in range(n):
           AX[:,:,ii]=np.matmul(A[:,:,ii],X)        
           
        #Centroid of transformations t and that
        t=1/n*np.sum(AX[0:3,3,:],1);
        that=1/n*np.sum(B[0:3,3,:],1);
        AXp[0:3,3,:]=AX[0:3,3,:]-np.tile(t[:,np.newaxis], (1, n))
        Bp[0:3,3,:]=B[0:3,3,:]-np.tile(that[:,np.newaxis], (1, n))

        [i,j,k]=AX.shape; #4x4xn
        #Convert AX and B to 2D arrays
        AXp_2D=AXp.reshape((i,j*k)) # now it is 4x(4xn)
        Bp_2D=Bp.reshape((i,j*k))# 4x(4xn)        
        #%Calculates the best rotation
        U, S, Vt=np.linalg.svd(np.matmul(Bp_2D[0:3,:],AXp_2D[0:3,:].T))# v is v' in matlab  
        R_est = np.matmul(Vt.T, U.T)
        # special reflection case
        if np.linalg.det(R_est) < 0:
            print ('Warning: Y_est returned a reflection')
            R_est =np.matmul( Vt.T, np.matmul(np.diag([1,1,-1]),U.T))       
        #Calculates the best transformation
        t_est = t-np.dot(R_est,that)
        Y_est[0:3,0:3]=R_est
        Y_est[0:3,3]=t_est
        #Calculate registration error
        pYB=np.matmul(R_est,B[0:3,3,:])+np.tile(t_est[:,np.newaxis],(1,n))#3xn
        pAX=AX[0:3,3,:]
        Reg_error=np.linalg.norm(pAX-pYB,axis=0) #1xn
        ErrorStats[0]=np.mean(Reg_error)
        ErrorStats[1]=np.std(Reg_error)
        return Y_est, ErrorStats